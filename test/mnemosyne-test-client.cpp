...
void
periodicAddRecord(KeyChain &keychain, shared_ptr<svs::SVSPubSub> pubSub, const Name &peerPrefix, Scheduler &scheduler) {
    ...
    scheduler.schedule(time::seconds(5), [&keychain, pubSub, peerPrefix, &scheduler] {
        periodicAddRecord(keychain, pubSub, peerPrefix, scheduler);
    });
}
